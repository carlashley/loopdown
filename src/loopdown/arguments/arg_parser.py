...
from .arg_helpers import MISSING
...
class StrictArgumentParser(argparse.ArgumentParser):
    ...
    Additional implementations of:
        ...
        - handle sentinels for 'nargs="?"' arguments via const=AUTO"""

    def __init__(self, *args, **kwargs) -> None:
        kwargs.setdefault("formatter_class", QuotedChoicesHelpFormatter)
        super().__init__(*args, **kwargs)

        self.any_required_groups: list[_AnyRequiredGroup] = []
        self.all_exclusive_groups: list[_AllExclusiveGroup] = []
    ...
    def add_any_required_group(
        ...
        *,
        ...
        self.any_required_groups.append(meta)
    ...
    def add_exclusive_group(
        ...
        *,
        ...
        grp.add_argument = add_argument  # type: ignore[assignment]
        self.all_exclusive_groups.append(meta)
    ...
    def parse_args(self, args: Sequence[str] | None = None, namespace: _N | None = None) -> argparse.Namespace | _N:
        ...
        self._validate_mirror_server_value(ns_for_validation)
    ...
    def _is_present(self, ns: argparse.Namespace, action: argparse.Action) -> bool:
        ...
        Works for:
            ...
            - store (default None)
            - nargs="?" with const sentinel AUTO
    ...
    def _validate_all_exclusive_groups(self, ns: argparse.Namespace) -> None:
        ...
        for grp in self.all_exclusive_groups:
    ...
    def _validate_any_required_groups(self, ns: argparse.Namespace) -> None:
        ...
        for grp in self.any_required_groups:
    ...
    def _validate_root_required(self, ns: argparse.Namespace) -> None:
        """Require effective UID 0 (root) unless arguments that don't require it are specified. Early return in
        dry-run or download mode because sudo is not required in those instances."""
        if getattr(ns, "dry_run", False) or getattr(ns, "action", "") == "download":
            return None

        if geteuid() != 0:
            self.error("you must be root (or run with sudo) to use this command unless -n/--dry-run is set")

    def _validate_mirror_server_value(self, ns: argparse.Namespace) -> None:
        """Validate the mirror server argument was given a value, after exclusive group check."""
        for grp in self.all_exclusive_groups:
            for action in grp.actions:
                if getattr(action, "const", None) is not MISSING:
                    continue

                if getattr(ns, action.dest, None) is MISSING:
